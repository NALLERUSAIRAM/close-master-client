...
export default function CloseMasterGame() {
  ...
  if (!game) return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-black text-white p-6">
      ...
      <div className="w-full max-w-xs space-y-4">
        <input className="w-full p-4 bg-gray-900 rounded-xl border border-white/10 text-lg font-bold outline-none" placeholder="Name" value={playerName} onChange={e => setPlayerName(e.target.value)} />
        <input className="w-full p-4 bg-gray-900 rounded-xl border border-white/10 text-center uppercase tracking-widest text-xl font-black outline-none" placeholder="ROOM ID" value={joinCode} onChange={e => setJoinCode(e.target.value)} />
  ...
  return (
    <div className="h-[100dvh] w-full text-white flex flex-col items-center p-2 overflow-hidden select-none relative font-sans">
      ...
      {/* 1. Header & Players Grouped at Top */}
      <div className="w-full max-w-md flex flex-col gap-1">
        <div className="w-full flex justify-between items-center bg-black/70 p-1.5 rounded-xl border border-white/10 backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 gap-1.5 w-full">
          {game.players.map(p => (
            <div key={p.id} className={`px-2 py-1.5 rounded-xl border ${game.turnId === p.id ? 'border-yellow-400 bg-yellow-400/20' : 'border-white/5 bg-black/50'}`}>
              <p className="font-black text-[8px] uppercase truncate opacity-60 mb-1">{p.name}</p>
      ...
      {/* 2. Middle Card - Positioned closer to players */}
      {game.started && (
        <div className="flex-1 flex flex-col items-center justify-start mt-10">
          ...
          {game.penaltyCount > 0 && <p className="mt-1 px-2 py-0.5 bg-red-600 rounded-full text-[8px] font-black animate-bounce">+{game.penaltyCount} PENDING</p>}
        </div>
      )}

      {/* 3. Action Area - Pulled Up from bottom */}
      <div className="w-full max-w-md pb-4 mt-auto mb-2">
        ...
        {myTurn ? (
          ...
          <button onClick={() => socket.emit("start_round", { roomId: game.roomId })} className="w-full py-4 bg-emerald-600 rounded-2xl font-black text-lg">START GAME</button>
        )}
      </div>
      ...
      {showResult && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 backdrop-blur-sm">
          <div className="w-full max-w-xs text-center">
            ...
            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 mb-4 max-h-[40vh] overflow-y-auto font-bold">
              {game.players.map(p => (
                <div key={p.id} className="flex justify-between py-2 border-b border-white/5 last:border-0 uppercase italic">
                  <span className="text-xs text-gray-300">{p.name}</span>
                  <span className="text-base text-emerald-400">+{p.lastRoundPoints}</span>
                </div>
              ))}
            </div>
            <button onClick={() => { setShowResult(false); if(game.hostId === game.youId) socket.emit("start_round", { roomId: game.roomId }); }} className="w-full py-3 bg-emerald-600 rounded-xl font-black text-base uppercase">Next Round</button>
          </div>
        </div>
      )}

      {showHistory && (
        <div className="fixed inset-0 z-[110] bg-black/95 flex items-center justify-center p-2 backdrop-blur-sm">
          <div className="bg-gray-900 w-full max-w-xs rounded-2xl border border-amber-500/30 p-3 max-h-[80vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="flex justify-between items-center mb-3 px-1 font-black italic"><h3 className="text-base text-amber-500 uppercase">Score Board</h3><button onClick={() => setShowHistory(false)} className="text-2xl text-gray-500">&times;</button></div>
            <div className="overflow-auto flex-1 italic text-[10px] font-bold">
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-gray-900 border-b border-white/10 text-gray-400 uppercase"><tr><th className="pb-1">RD</th>{game.players.map(p => <th key={p.id} className="pb-1">{p.name}</th>)}</tr></thead>
                <tbody>
                  ...
                  <tr className="text-amber-400 text-sm font-black uppercase sticky bottom-0 bg-gray-900 pt-2 border-t border-white/10"><td>Total</td>{game.players.map(p => <td key={p.id}>{p.score}</td>)}</tr>
                </tbody>
              </table>
            </div>
          </div>
