...
const Card = ({ card, isSelected, onClick, isMiddle }) => (
  <div onClick={onClick} className={`relative bg-black/95 border-2 rounded-xl flex flex-col justify-between p-2 transition-all cursor-pointer select-none ${getCardStyle(card)} ${isMiddle ? 'w-28 h-40 shadow-2xl' : 'w-20 h-32 md:w-24 md:h-36'} ${isSelected ? '-translate-y-8 scale-110 z-20 !border-white shadow-[0_0_20px_white]' : ''}`}>
    ...
    <div className="text-4xl self-center drop-shadow-md">{card.rank === "JOKER" ? "🃏" : card.suit}</div>
    ...
    <div className="min-h-screen text-white flex flex-col items-center p-4 overflow-hidden select-none font-sans">
      ...
      <div className="w-full max-w-2xl flex justify-between items-center bg-black/70 p-4 rounded-3xl border border-white/10 mb-4 backdrop-blur-md">
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowHistory(true)} className="px-5 py-2 bg-amber-500 rounded-xl font-black text-xs shadow-lg">📊 POINTS</button>
          <button onClick={() => setBg(BG_THEMES[bg.id === "t15" ? 1 : 0])} className="p-2 bg-white/10 rounded-xl border border-white/10 text-xl">🎨</button>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 w-full max-w-2xl mb-6">
        {game.players.map(p => (
          <div key={p.id} className={`p-4 rounded-2xl border-2 transition-all ${game.turnId === p.id ? 'border-yellow-400 bg-yellow-400/20 scale-105 shadow-xl' : 'border-white/5 bg-black/50'}`}>
      ...
      <div className="mt-auto w-full max-w-4xl pb-6">
        ...
        {myTurn ? (
          <div className="grid grid-cols-3 gap-3 px-2 max-w-lg mx-auto">
            ...
            <button onClick={() => { if(window.confirm("CLOSE?")) socket.emit("action_close", { roomId: game.roomId }); }} className="py-5 bg-pink-600 rounded-2xl font-black text-xl shadow-xl uppercase tracking-tighter">Close</button>
          </div>
            ...
            <div className="bg-white/5 rounded-[32px] p-8 border border-white/10 mb-10 shadow-2xl">
              {game.players.map(p => (
                <div key={p.id} className="flex justify-between py-4 border-b border-white/5 last:border-0">
                  <span className="text-2xl font-bold text-gray-300 uppercase">{p.name}</span>
                  <span className="text-3xl font-black text-emerald-400">+{p.lastRoundPoints}</span>
                </div>
              ))}
            </div>
