...
export default function App() {
  ...
  // Hide status bar when app opens
  useEffect(() => {
  ...
  const games = [
    ...
    },
    {
      id: "cards_show",
      name: "Cards Show",
      desc: "13 Unique Cards Challenge",
    ...
    },
    {
      id: "set_show",
      name: "Set Show",
      desc: "3x3 & 1x4 Sets with Secret Bonus",
    ...
    return (
      <div className="h-[100dvh] w-full flex flex-col items-center justify-center text-white p-6 font-sans relative overflow-hidden">
        ...
        <form
          ...
          className="w-full max-w-sm z-10 bg-black/60 backdrop-blur-xl p-8 rounded-[32px] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col gap-6 relative group"
        >
          <div className="flex flex-col gap-2 text-center relative z-10">

            <label className="text-[11px] uppercase tracking-[0.25em] text-yellow-400/90 font-black">
    ...
    return (
      <div className="h-[100dvh] w-full flex flex-col items-center justify-start text-white p-4 sm:p-8 font-sans overflow-y-auto relative">
        ...
        <div className="w-full max-w-4xl flex justify-between items-center mb-8 bg-black/60 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-lg z-10">
          ...
          <button
            onClick={handleBack}
            className="text-gray-400 hover:text-white font-bold px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl transition"
          >
            ⬅ BACK
          </button>

          <div className="text-center flex-1">
            <span className="text-yellow-400 font-black text-2xl italic tracking-tighter uppercase">
              Gully Cards Hub
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 bg-black/60 px-4 py-2 rounded-xl border border-yellow-400/30">
            ...
            <span className="text-sm text-yellow-400 font-black truncate max-w-[100px]">
              {playerName}
            </span>
          </div>
        </div>

        <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-widest mb-8 text-gray-200 z-10">
          Select Your Deck
        </h2>
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 px-4 z-10">
          ...
          {games.map((game) => (
            <div
              ...
              className={`relative group cursor-pointer flex flex-col items-center bg-gradient-to-b ${game.color} p-1 rounded-3xl transition-all duration-300 hover:-translate-y-4 hover:scale-105 shadow-xl`}
            >
    ...
    return (
      <div className="h-[100dvh] w-full flex flex-col items-center justify-center text-white p-6 font-sans relative">
        ...
        <div className="absolute top-6 left-6 z-10">
          <button
            onClick={handleBack}
            className="text-gray-400 hover:text-white font-bold px-4 py-2 bg-black/60 rounded-xl border border-white/10 transition"
          >
            ⬅ DECKS
          </button>
        </div>
        ...
        <div className="text-center mb-8 z-10">
          ...
          <h2
            className={`text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r ${selectedGame.color} uppercase italic drop-shadow-lg`}
          >
            {selectedGame.name}
          </h2>

          <p className="text-sm uppercase tracking-widest text-gray-300 font-bold mt-2">
            {selectedGame.desc}
          </p>
        </div>
        ...
        <div className="w-full max-w-sm bg-black/60 backdrop-blur-md p-8 rounded-3xl border border-white/20 shadow-2xl flex flex-col gap-6 z-10">
          ...
          <button
            onClick={() => {
              setRoomAction({ type: "create", code: "" });
              setStep("playing");
            }}
          ...
          <div className="relative flex items-center justify-center my-2">
            ...
            <span className="absolute bg-[#12141f] px-4 text-xs font-bold text-gray-400 uppercase tracking-widest">
              OR
            </span>
          </div>
